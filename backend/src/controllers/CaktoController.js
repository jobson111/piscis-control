// backend/src/controllers/CaktoController.js (VERSÃO FINAL)

const db = require('../config/db');
const registrarLog = require('../helpers/logHelper');
const axios = require('axios');


const caktoApi = axios.create({
    baseURL: 'https://api.cakto.com.br/v1', // URL base da API da Cakto
    headers: {
        'Authorization': `Bearer ${process.env.CAKTO_API_KEY}`,
        'Content-Type': 'application/json'
    }
});

exports.createCheckoutLink = async (req, res) => {
    const { pisciculturaId, email, nome } = req.user;
    const { preco_id } = req.body;
    const client = await db.pool.connect();
    try {
        await client.query('BEGIN');
        const pisciculturaResult = await client.query('SELECT * FROM pisciculturas WHERE id = $1', [pisciculturaId]);
        if (pisciculturaResult.rowCount === 0) throw new Error('Piscicultura não encontrada.');
        
        const piscicultura = pisciculturaResult.rows[0];
        let gatewayCustomerId = piscicultura.gateway_customer_id;

        if (!gatewayCustomerId) {
            const caktoCustomerPayload = { name: nome, email: email, doc_number: piscicultura.cnpj || '' };
            const customerResponse = await caktoApi.post('/customers', caktoCustomerPayload);
            gatewayCustomerId = customerResponse.data.id;
            await client.query('UPDATE pisciculturas SET gateway_customer_id = $1 WHERE id = $2', [gatewayCustomerId, pisciculturaId]);
        }

        const precoResult = await client.query("SELECT gateway_id FROM precos_planos WHERE id = $1", [preco_id]);
        if (precoResult.rowCount === 0 || !precoResult.rows[0].gateway_id) throw new Error('Link de pagamento não configurado.');
        
        const gatewayOfferId = precoResult.rows[0].gateway_id;
        const checkoutLink = `https://pay.cakto.com.br/${gatewayOfferId}?customer_id=${gatewayCustomerId}`;
        
        await client.query('COMMIT');
        res.status(200).json({ checkoutUrl: checkoutLink });
    } catch (error) {
        await client.query('ROLLBACK');
        console.error("Erro ao criar link de checkout:", error.response ? error.response.data : error.message);
        res.status(500).json({ error: 'Erro ao comunicar com o sistema de pagamento.' });
    } finally {
        client.release();
    }
};

exports.handleWebhook = async (req, res) => {
    const webhookData = req.body;
    const nossoTokenSecreto = process.env.CAKTO_WEBHOOK_SECRET;

    if (webhookData.secret !== nossoTokenSecreto) {
        return res.status(403).json({ error: 'Token inválido.' });
    }

    const eventType = webhookData.event;
    console.log(`✅ Webhook da Cakto verificado. Evento: ${eventType}`);

    try {
        // Focamos nos eventos que confirmam uma assinatura paga
        if (eventType === 'subscription_activated' || eventType === 'sale_approved') {
            const dadosDoEvento = webhookData.data;
            const emailCliente = dadosDoEvento.customer.email;
            const idOfertaCakto = dadosDoEvento.offer.id;
            
            // A Cakto deve enviar o ID do cliente deles no payload, vamos procurar por ele
            const gatewayCustomerId = dadosDoEvento.customer.id || dadosDoEvento.customer.doc_number; // Adapte para o campo correto

            if (!gatewayCustomerId) throw new Error("ID de cliente da Cakto não encontrado no webhook.");

            console.log(`--- [AÇÃO] Processando assinatura para o cliente Cakto [${gatewayCustomerId}]`);
            
            const precoResult = await db.query("SELECT id, plano_id, ciclo_cobranca FROM precos_planos WHERE gateway_id = $1", [idOfertaCakto]);
            if (precoResult.rowCount === 0) throw new Error(`ID de oferta da Cakto [${idOfertaCakto}] não encontrado.`);
            
            const { id: preco_id, plano_id, ciclo_cobranca } = precoResult.rows[0];
            let intervalo = ciclo_cobranca === 'ANUAL' ? '1 year' : '1 month';

            const updateResult = await db.query(
                `UPDATE pisciculturas SET plano_id = $1, preco_id = $2, status_assinatura = 'ATIVO', data_expiracao_assinatura = NOW() + interval '${intervalo}' WHERE gateway_customer_id = $3`,
                [plano_id, preco_id, gatewayCustomerId]
            );

            if (updateResult.rowCount === 0) throw new Error(`Piscicultura com gateway_customer_id [${gatewayCustomerId}] não encontrada.`);
            
            console.log(`✅ SUCESSO: Assinatura da piscicultura foi ativada.`);
        }
        res.status(200).json({ received: true });
    } catch (error) {
        console.error("❌ ERRO CRÍTICO AO PROCESSAR WEBHOOK:", error.message);
        res.status(500).json({ error: 'Erro interno no processamento do webhook.' });
    }
};